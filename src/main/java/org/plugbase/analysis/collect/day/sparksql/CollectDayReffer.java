...
public class CollectDayReffer {
	private static final Logger log = Logger.getLogger(CollectDayReffer.class);
	...
	public static void collect(String inPath, String outPath) {
		try {
			...
			JavaPairRDD<String, CountReffer> hitData = null;
			
			if (HadoopFileSystem.isExistsFile(outPath)) {
				hitData = sqlctx.read()
						.parquet(outPath)
						.javaRDD()
						.mapToPair(row -> RefferFunc.mapToPair(row))
						.cache();
				hitData.count();
			}
			...
			for (FileStatus fs : fileStatus) {
				...
				JavaPairRDD<String, CountReffer> tmpData = sqlctx.read()
						.parquet(newFilePath)
						.javaRDD()
						.mapToPair(row -> RefferFunc.mapToPair(row));
			...
			JavaRDD<CountReffer> refferDayData = hitData.reduceByKey((v1, v2) -> RefferFunc.reduceByKey(v1, v2)).values();
			
			sqlctx.createDataFrame(refferDayData, CountReffer.class).write().mode(SaveMode.Overwrite).parquet(outPath);

			for (String path : finshPaths) {
				HadoopFileSystem.delFile(path);
			}
		} catch (Exception e) {
			log.error("collect day reffer fail",e);
		}
	}
}
